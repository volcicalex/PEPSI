...
TEST(Mementis, EgalitePrix) {
	...
	pnl_vect_set(trend_, 1, mementis->taux_capitalisation_ );
	pnl_vect_set(trend_, 2, mementis->taux_capitalisation_ );
	...
	PnlVect *sigmaChangeRate = pnl_vect_create_from_scalar(2, 0.01);
	PnlVect *spotChangeRate = pnl_vect_create_from_scalar(2,1.8);
	PnlVect *spotRiscklessAsset = pnl_vect_create_from_scalar(2, 1);
	...
	BlackScholesModel *bsmodel_CR = new BlackScholesModel(nbAssets, nbMarkets, mementisCR->size_, nbAssetsPerMarket_, sigmaChangeRate, mementisCR->taux_capitalisation_, rho_CR, sigma, spot, spotChangeRate, spotRiscklessAsset, trend_);
